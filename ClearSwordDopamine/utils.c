...
int offsets_init(void) {
    ...
    } else if (OS_VERSION_AT_LEAST(18, 7)) {
        // iOS 18.7.x offsets for A12 (iPhone XS)
        // Based on kernelcache analysis: xnu-11417.140.69.704.2
        g_offsets.inpcb_icmp6filt = 0x148;
        g_offsets.socket_so_count = 0x254;
        g_offsets.socket_so_background_thread = 0x2b0;
        g_offsets.thread_t_ro = 0x378;  // A12 specific for 18.7.x
        g_offsets.thread_ro_proc = 0x18;
        LOG("Using iOS 18.7.x offsets for A12");
